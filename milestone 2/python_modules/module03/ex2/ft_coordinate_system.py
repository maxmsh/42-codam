...
def get_player_pos():
    while True:
        ...
        try:
            ...
            distance_rounded: float = round(distance_to_center, 4)
...
if __name__ == "__main__":
    ...
    distance_rounded: float = get_player_pos()
    print("\nGet a second set of coordinates")
    while True:
        raw: str = input("Enter new coordinates as floats in format 'x,y,z': ")
        try:
            x, y, z = map(float, raw.split(','))
            distance_to_center_second: float = math.sqrt((x) **2 + (y) **2 + (z) **2)
            distance_rounded_second: float = round(distance_to_center_second, 4)
            distance_diff: float = distance_rounded_second - distance_rounded
